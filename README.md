...
Feed Goblin isn't on the Chrome Web Store — you add it to your browser yourself, once, in about a minute. It's the standard "Load unpacked" flow; just follow the steps.

This repository (`github.com/jakchagu/feed-goblin`) is the only place I've uploaded it — there's no Chrome Web Store or Firefox listing.

1. **Download it.** Click the green **`<> Code`** button (top of this page) → **Download ZIP**. *(Or clone the repo, if you use git.)*
2. **Unzip it — don't just double-click it.** Double-clicking a ZIP only *peeks inside* it (you'll see the files, but they're still zipped — this is the #1 spot people get stuck). Instead: **right-click the downloaded ZIP → Extract All...** → **Extract.** ("Extract All..." is partway down the menu — *not* the "Open" options at the top, which just take you inside the still-zipped archive.) The Extract dialog's suggested location is fine — just click **Extract**. When it finishes, the real unzipped folder opens on its own — that's the one you'll use. **Leave that folder where it lands** — Chrome runs the extension from it, so moving or deleting it later stops it working.
3. **Open your extensions page** — go to `chrome://extensions` (or `edge://extensions`).
4. **Turn on Developer mode** — the toggle in the top-right corner.
5. **Click "Load unpacked"** and pick the **unzipped** folder — the one with **`manifest.json`** directly inside it. (If you open the folder and don't see `manifest.json`, go one level deeper and pick *that* folder — downloaded ZIPs sometimes nest it.)
6. **Pin it** so the popup is easy to reach (puzzle-piece icon → pin).
